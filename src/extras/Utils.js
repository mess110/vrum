class Utils {
  ...
  static box(options) {
    ...
    if (isBlank(options.size)) { options.size = 1 }
    if (isBlank(options.color)) { options.color = '#4d4d4d' }

    let geometry = new THREE.BoxGeometry(options.size, options.size, options.size)
  ...
  static boundingBox(options) {
    let box = Utils.box(options)
    box.material.visible = Config.instance.engine.debug
    return box
  }
...
`
  ...
  static toggleOrbitControls(target) {
    ...
    let orbitControls = Hodler.get('orbitControls')
    if (orbitControls !== null && orbitControls !== undefined) {
      ...
      orbitControls = new (target)(camera, renderer.domElement)
  ...
  static toggleStereoEffect() {
    ...
    let renderer = Hodler.get('renderer')
    let rendererDefault = Hodler.get('rendererDefault')
  ...
  static screenshotContents(canvas, withMeta) {
    ...
    let screenshotData = canvas.toDataURL()
...
display: none; \
  ...
  static addCEButton(options) {
    ...
    if (!options.size.endsWith('px')) {
      options.size = options.size + "px";
    }
    if (!options.padding.endsWith('px')) {
      options.padding = options.padding + "px";
    }
    if (!Utils.CE_BUTTON_TYPES.includes(options.type)) {
      throw new Error("invalid type " + options.type);
    }
    if (document.querySelector(".ce-button-" + options.type)) {
      return;
    }
    if (!Utils.CE_BUTTON_POSITIONS.includes(options.position)) {
    ...
    if (options.type === 'fullscreen') {
      ...
      img.setAttribute('onclick', 'Engine.switch(Hodler.get("scene"))');
  ...
  static removeCEButtons() {
    ['.ce-button-fullscreen', '.ce-button-reinit'].forEach((className) => {
